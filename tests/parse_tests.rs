...
use vir_search::parse::{parse, parse_with_resolver, PerspectiveResolver};

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum TestField { Genre, Artist, Album, Title, Rating, Year, Added, Format, Duration, Author, Narrator, Series }
impl std::fmt::Display for TestField {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            Self::Genre => "genre", Self::Artist => "artist", Self::Album => "album", Self::Title => "title",
            Self::Rating => "rating", Self::Year => "year", Self::Added => "added", Self::Format => "format",
            Self::Duration => "duration", Self::Author => "author", Self::Narrator => "narrator", Self::Series => "series",
        };
        write!(f, "{s}")
    }
}
impl ParseField for TestField {
    fn parse(name: &str) -> Option<Self> {
        match name {
            "genre" => Some(Self::Genre), "artist" => Some(Self::Artist), "album" => Some(Self::Album),
            "title" => Some(Self::Title), "rating" => Some(Self::Rating), "year" => Some(Self::Year),
            "added" => Some(Self::Added), "format" => Some(Self::Format), "duration" => Some(Self::Duration),
            "author" => Some(Self::Author), "narrator" => Some(Self::Narrator), "series" => Some(Self::Series),
            _ => None,
        }
    }
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum TestState { Starred, Finished }
impl std::fmt::Display for TestState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", match self { Self::Starred => "starred", Self::Finished => "finished" })
    }
}
impl ParseState for TestState {
    fn parse(name: &str) -> Option<Self> {
        match name { "starred" => Some(Self::Starred), "finished" => Some(Self::Finished), _ => None }
    }
}
...
impl ParseSort for TestSort {
    fn parse(name: &str) -> Option<Self> {
        match name { "added" => Some(Self::Added), _ => None }
    }
}
...
fn extracts_sort() {
    ...
    assert_eq!(p.sorts, vec![SortSpec { key: TestSort::Added, descending: true }]);
    assert_eq!(p.expr, parse::<TestField, TestState, TestSort>("genre:ambient").expr);
}
...
fn empty_input_is_empty() {
    assert_eq!(parse::<TestField, TestState, TestSort>("").expr, Expr::Empty);
    assert_eq!(parse::<TestField, TestState, TestSort>("   ").expr, Expr::Empty);
}
...
fn vl_expands_via_resolver() {
    ...
    assert_eq!(p.expr, parse::<TestField, TestState, TestSort>("genre:ambient AND rating:>=4").expr);
    assert!(p.warnings.is_empty());
}
...
fn vl_without_resolver_degrades_to_text() {
    assert_eq!(parse::<TestField, TestState, TestSort>("vl:fav").expr, Expr::Text("vl:fav".into()));
}
