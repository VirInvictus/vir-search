...
*Context: Found critical AST parsing flaws, edge cases causing catastrophic parse failure, and documentation desyncs during a full codebase sweep.*
...
- [x] **Docs/API:** README's "resolved during parsing using chrono" is
      ...
      clippy gate.)*
- [ ] **Consumer-doc drift (cross-repo lens; executed under grant #117):**
      Refreshed 2026-09-15: Conservatory's search-grammar.md keyword list
      was already fixed on its side (317ee7d). Still live: Atrium's spec
      documents Ndaysout (unparseable) and a quoted-exact form
      (`tag:"=x y"`) that parses as literal substring (Exact needs
      `tag:="x y"`); Conservatory's spec.md:199 teaches "is:finished
      false"; Conservatory's roadmap attributes SQL translation to
      vir-search's sql_translate, a module that has never existed; and
      search-grammar.md's release attributions are wrong
      (tomorrow/lastweek/nextweek shipped 1.0.4, lastmonth/nextmonth
      1.4.1) while omitting `in<N>days`/`±Nd` and the wildcard/in-list
      matchers. Small doc commits in each closed repo, logged in both
      records; the attribution fix matters before any push-down work. *(Executed 2026-09-15: Atrium `c2562bd` fixes its spec quoted-exact row (`tag:="x y"`, with the literal-substring explanation) and names the real forward-day spellings; Conservatory `256bfed` corrects the push-down box's sql_translate attribution to its own search layer and the search-grammar date-keyword attributions plus the missing relative-offset forms; the spec `is:finished` example was already fixed by Conservatory's own lane, recorded as an audit correction.)*
- [ ] **Blitz candidates:** diagnostic ergonomics (severity,
      suggestions, line/col: the spans shipped so search bars could draw
      squiggles and no consumer consumes them; the perspective-span
      coordinate system is now documented for it); the SQL push-down
      scaffold (MatchKind shapes + walker in-library; per-schema
      fragments stay consumer-side; recorded deferral sequenced with
      future consumer waves); QueryCache hit-path tuning only once a
      consumer adopts it. *(The round-trip fuzz shipped 1.4.3.)*
...
- [x] MED: Display round-trip flips semantics for grammar-intercepted bare words: quote_if_needed doesn't quote true/false/`*` values, so `genre:"true"` (Substring by the 1.0.4 rule) renders bare and re-parses as HasAny/HasNone/Prefix/Suffix (ast.rs:399-416). Add the cases + corpus + semantic assertion; extract the shared boundary helper (is_boundary vs quote_if_needed's closure are hand-synced). *(Shipped 1.4.3: quote_if_needed quotes true/false in any case and the prefix/suffix wildcard shapes, mirroring the parser's unquoted-value arms through the shared `lex::is_boundary`/`ast::bool_word` helpers instead of hand-synced copies; corpus entries plus an end-to-end semantic test pin `genre:"true"` and friends.)*
- [x] MED: Quoted In-list contradicts four "quoted is always literal" doc statements (README:42/:26, spec:53, roadmap:16) and breaks round-trip when an item contains a boundary (`genre:("hip hop,ambient")` renders unquoted, re-parses to malformed Text). Decide reject-vs-document; per-item quoting in Display. *(Shipped 1.4.3: decided reject (gate Q1, 2026-09-15); a quoted body after `(` degrades to its visible fragment with a warning, the unquoted list is untouched, and the README/spec/guidance state the rule explicitly so the absolutes hold as written.)*
- [x] MED: resolve_vl merges sub-parse diagnostics whose byte spans index the perspective's stored text, not the outer query (spans can exceed the query length; parse.rs:660-664). Document the coordinate system before the diagnostic-ergonomics lane builds on spans. *(Shipped 1.4.3: the perspective-text coordinate system is documented on `Diagnostic`, `parse_with_resolver`, and spec 2.3; remapping into outer coordinates stays with the diagnostic-ergonomics lane.)*
- [x] MED: regex dependency has zero use sites over the crate's entire life (structural: parser+AST-only charter). Removal needs sign-off + 1.4.3-class release + doc sync (spec:5, CLAUDE:3) + consumer lock hop. *(Shipped 1.4.3: removed per gate Q2 sign-off 2026-09-15; spec.md:5 and the guidance dep line synced; the lock shrinks five packages.)*
...
- [ ] MED: GitHub: main unprotected while both consumers pin by rev (block force-push + deletion, nothing else); declared MSRV 1.85 verified by nothing (add a pinned 1.85.0 CI job); no tag trigger, no CI badge, private vulnerability reporting off. *(Blitz 2026-09-15: the tag trigger (`b9b41d2`), the README badge, the CI `permissions:` block, and private vulnerability reporting are done, the last verified reading back `enabled: true`; the ruleset and the MSRV job were asked (2026-09-15, gate round 2) and are pending Brandon's answers, so they stay unexecuted by the gate rule.)*
- [x] LOW: ymd 1970 fallback can invert ranges at the representable edge (`added:262143` → start > end; `added:<262143` matches everything, warning-free). Clamp the year at parse (range decision: 0..=9999 vs chrono max) or saturate the fallback. *(Shipped 1.4.3: parse-time validation with the valid range 0..=9999 per gate Q3, 2026-09-15; out-of-range years degrade to visible text with the bad-value warning instead of resolving through the 1970 fallback.)*
- [x] LOW: Negation-cap off-by-one (65 marks says "exceed the cap; keeping 65" vs the at-most-64 doc); balanced-empty group eats its closer then warns "unclosed parenthesis"; And-under-And renders unparenthesized (shape-level round-trip break, semantics preserved). *(Shipped 1.4.3: the MAX_NEGATIONS rustdoc states the parity rounding explicitly, at most 65 kept; a balanced-empty group claims its closer silently instead of warning "unclosed"; Display parenthesizes nested And-under-And and Or-under-Or so shape round-trips.)*
- [x] LOW: PerspectiveResolver trait doc false about plain parse (no resolver attached; vl: degrades silently); module header "every degradation reported twice" false for the resolver-less arm; rank.rs:33 comment on the wrong arm (dissolve via Expr::visit rewrite); fold.rs norm_key cross-repo cite + dates.rs CalibreQuarry attribution + today_utc doc on the wrong function + add_month precondition doc missing; In-list "quoted is literal" comment invites the false reading at that position; stray-closer test comment predates the 1.4.2 warning; fuzzy threshold doc "1-4" vs the 0..=4 arm; lex.rs Token doc duplicates the module header. *(Shipped 1.4.3: the full comment truth batch, including the module-header reported-twice boundary, the rank.rs comment moved to the arm that excludes, dates.rs self-contained semantics, add_month's precondition, the fuzzy up-to-4 band, the lex.rs Token doc, and the fold.rs FTS5 claim softened to the verified truth.)*
- [x] LOW: Normative grammar table gaps: date-keyword examples need the field prefix on every row (bare words parse as Text); juxtaposition (implicit AND) row missing; kb suffix and sort:+key forms missing; spec AST notation is tuple-style vs named-field variants; vacuous malformed-list test assertion (compares parse(x) to itself); CLAUDE.md chrono claim (chrono enters only at resolve_range). *(Shipped 1.4.3: every date-keyword example carries the field prefix, a juxtaposition row documents implicit AND, the kb suffix and sort:+key are documented, spec 2.1 writes named-field variants with a rustdoc pointer, the vacuous malformed-list assertion pins Expr::Text, and CLAUDE.md states chrono enters only at resolve_range.)*
- [x] LOW: fold's U+0300-036F strip may not match FTS5 unicode61 remove_diacritics=2 beyond Latin (verify or soften the dual-path claim); ParseResult/QueryCache missing Debug derives; unknown-field degradation eats only the plain value (bogus:>=5 → three text nodes); warnings uncapped on pathological input ("("*50000 → ~49.8k duplicates). *(Shipped 1.4.3: verified against FTS5 directly, sqlite 3.51.2, and the direction surprised: fold strips all of U+0300-036F while FTS5's table is narrower (a Cyrillic й keeps its breve on the SQL side); Hebrew niqqud and Arabic harakat are untouched by both. The fold.rs claim is softened to the verified truth; extending the strip set stays parked until a consumer asks. ParseResult/QueryCache derive Debug; unknown-field degradation keeps the comparator and value; the degradation log caps at 100 entries, 3 copies per message, with a suppressed-count summary.)*
- [ ] LOW: Housekeeping: logo.svg absent vs house standard (add or declare); CI lacks a permissions block; Cargo.toml missing readme/publish=false (pin the git-deps-only model); roadmap consumer-drift box (:81-88) itself stale post today's Conservatory fixes. *(Blitz 2026-09-15: the CI permissions block landed with the GitHub pass (`b9b41d2`), API.md was created (the workspace doc set requires it of every shared library), and the consumer-drift box is refreshed; logo-or-declare and the Cargo.toml readme/publish pins were asked (2026-09-15, gate round 2) and are pending Brandon's answers.)*
- [x] Feature candidates logged (FINAL-REPORT L4, ranked): generated-input round-trip fuzz (no-dep, closes the twice-shipped bug class); regex removal + Ymd validation + examples/mini_consumer + warnings cap as one 1.4.3 hop; SQL push-down scaffold (sequenced with consumer waves, after the Conservatory record fix); diagnostic ergonomics (prepared lane); QueryCache tuning + streaming lexer (parked). Declined-by-default: bare 7d, Calibre-parity operators until a consumer asks. *(Shipped 1.4.3: the round-trip fuzz, 5000 seeded cases over grammar pieces whose first run caught the Not(Empty) and nested-combinator holes, and the hygiene hop (regex removal, Ymd validation, Debug derives, examples/mini_consumer.rs, the warnings cap). The SQL push-down scaffold stays a recorded deferral sequenced with future consumer waves; QueryCache tuning and the streaming lexer stay parked; diagnostic ergonomics remains the prepared next lane; bare 7d and Calibre-parity operators stay declined-by-default.)*
- [x] Prose pass: README register stratum (:5 "powerful … entire suite", :13 "gracefully degrade", :46 "Crucially"); roadmap.md:22 "rigorous" self-grade; roadmap em-dashes :28/:35 + six spaced-hyphen dashes in the 2026-09-12 block; spec.md:27 "crucially". Tag-frozen patchnotes dashes stay as-is. *(Shipped 1.4.3: the README superlative, "gracefully degrade", and "Crucially" are cut, the spec "crucially" is recast, the roadmap "rigorous" self-grade is dropped, and both em-dashes plus all six spaced-hyphen surrogates are recast; tag-frozen patchnotes dashes stay as-is.)*
...
## Cascade ledger: the 2026-09-15 consumer wave (CROSS-REPO GRANT #117)

Both consumers are synced to v1.4.3 in writing; the wave was one lock hop
carrying the whole final-audit lane, run after every library fix landed
(the sheet's own sequencing rule).

- **Atrium**: lock moved vir-search 1.4.2 @ `d3b28e4` to 1.4.3 @ `8087e93`
  (`c34639a`, with `data/cargo-sources.json` regenerated; the regex edge
  vir-search never used dropped from Atrium's lock too). Full workspace
  suite green locally; adoption line in patchnotes Unreleased. The spec
  §4.3.3 drift fixes rode the same wave (`c2562bd`).
- **Conservatory**: lock moved vir-search 1.4.2 @ `d3b28e4` to 1.4.3 @
  `8087e93` (`a3aa830`, with `data/cargo-sources.json` regenerated; same
  regex edge dropped). Workspace suite green locally; adoption line in a
  new patchnotes Unreleased section. The roadmap attribution and
  search-grammar fixes rode the same wave (`256bfed`).
- No waiver needed: nothing is outstanding. Viaduct is not a consumer.
