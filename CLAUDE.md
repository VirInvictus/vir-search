...
- **Round-Trip Integrity**: The `Display` implementation for `Expr<F, S>` must exactly recreate a parseable string that produces an identical AST. This ensures that user perspectives can be serialized to disk and loaded without semantic drift. Note the round-trip harness asserts shape-stability, not semantics: new grammar needs its own semantic assertions (the month-keyword gap slipped through exactly this way).
